...
```bash
cat >> /etc/hosts <<EOF
192.168.56.104 k8s-cluster-101
192.168.56.105 k8s-cluster-102
192.168.56.106 k8s-cluster-103
EOF
```
...
设置三台机机器的root密码`sudo passwd`

先在三个机器上配置允许远程登录

[linux下开启SSH，并且允许root用户远程登录,允许无密码登录](https://www.cnblogs.com/toughlife/p/5633510.html)

修改文件内容`/etc/ssh/sshd_config`，然后重启
...
```
ssh-keygen -t rsa
ssh-copy-id root@k8s-cluster-101
ssh-copy-id root@k8s-cluster-102
ssh-copy-id root@k8s-cluster-103
```
